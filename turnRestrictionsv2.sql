...
--	END LOOP;
	...
	FOR relation IN (
		...
		SELECT e.* INTO edge_from FROM edges_routing e JOIN edges_data_routing d ON e.data_id = d.id JOIN relation_members rm ON d.osm_id = rm.member_id WHERE (rm.relation_id = relation.id AND rm.member_role = 'from' AND (e.target_id = node.id OR e.source_id = node.id));
		SELECT e.* INTO edge_to FROM edges_routing e JOIN edges_data_routing d ON e.data_id = d.id JOIN relation_members rm ON d.osm_id = rm.member_id WHERE (rm.relation_id = relation.id AND rm.member_role = 'to' AND (e.target_id = node.id OR e.source_id = node.id));
		...
		IF relation.tags->'restriction' LIKE 'no_%' THEN
			...
			RAISE NOTICE 'idx: %, nodecount %', idx, node_count;
